<script lang="ts">
  ...
  import { dateKey, addDays, parseDate, DOW_SHORT } from '$lib/date';
  import Sheet from '$lib/components/Sheet.svelte';
  import RoutinePicker from '$lib/components/RoutinePicker.svelte';

  let cursor = $state(new Date());   // any day in the displayed month

  const monthStart = $derived(new Date(cursor.getFullYear(), cursor.getMonth(), 1));
  const monthEnd   = $derived(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 0));
  const gridStart  = $derived(addDays(monthStart, -((monthStart.getDay() + 6) % 7)));   // back to Monday
  ...
  // ─── recurring editor (weekly defaults) ────────────────────────────────────
...
<div class="grid grid-cols-7 gap-1">
  {#each cells as c (c.dk)}
    ...
    <button
      class="aspect-square card text-left p-1 flex flex-col {c.inMonth ? '' : 'opacity-30'}
             {isToday ? 'border-teal' : ''}"
      onclick={() => openEditor(c.dk)}
    >
      <div class="flex items-center justify-between">
        <div class="text-[11px] font-semibold {isToday ? 'text-teal' : 'text-text2'}">{c.date.getDate()}</div>
        {#if override}<div class="w-1 h-1 rounded-full bg-amber" title="Override"></div>{/if}
      </div>
      <div class="mt-auto flex flex-col gap-0.5">
        {#each routines.slice(0, 2) as r (r.id)}
          <div class="text-[8px] truncate {r.category === 'rehab' ? 'text-teal' : 'text-violet'}">{r.name}</div>
        {/each}
        {#if routines.length > 2}
          <div class="text-[8px] text-text3">+{routines.length - 2}</div>
        {/if}
      </div>
      {#if exs.length > 0}
